...
if [ "xxx$1" = "xxx" ]; then
  pkgs="certbot acme certbot_apache certbot_dns_cloudflare certbot_dns_cloudxns certbot_dns_digitalocean certbot_dns_dnsimple certbot_dns_dnsmadeeasy certbot_dns_google certbot_dns_luadns certbot_dns_nsone certbot_dns_rfc2136 certbot_dns_route53 certbot_dns_rackspace certbot_nginx letshelp_certbot"
else
  pkgs="$@"
fi
...
cover () {
  if [ "$1" = "certbot" ]; then
    ...
    min=92
  elif [ "$1" = "certbot_dns_rackspace" ]; then
    min=92
